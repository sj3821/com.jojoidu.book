...
public class HelloControllerTest {
    ...
    public void helloDto가_리턴된다() throws Exception {
        ...
        mvc.perform(get("/hello/dto")
                .param("name", name) // param : API 테스트 할때 사용될 요청 파라미터 설정, 단 값은 String만 허용. 숫자/날짜데이터는 등록할땐 문자열로 변경해야만 가능
                .param("amount", String.valueOf(amount)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.name", is(name))) // JSON 응답값을 필드별로 검증할 수 있는 메서드입니다. $를 기준으로 필드명을 명시. ex) $.name $.amount
